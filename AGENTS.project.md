...
2) Platform targeting supports iOS, macOS, tvOS, with bundle-ID overrides and platform-aware validation.
...
6) Presets are supported (built-in + custom) and can store platforms, template profile, bundle IDs, and GitHub visibility defaults.
...
9) Inline validation hints appear before Create for project name and bundle IDs.
...
14) Blank bundle IDs are treated as defaults derived from the current project name at create time and are not globally persisted across launches.
...
1) Validate project name and relevant bundle IDs first.
2) Create folder and project files.
3) Generate `.xcodeproj` and apply selected platform + bundle settings.
